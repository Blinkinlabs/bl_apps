...
PadConfig pincfg_table[] = 
{
  ...
  { // setup debug input pins - S3_SW_DP_CLK
    .ucMode = PAD_MODE_INPUT_EN,
    ...
    .ucSmtTrg = PAD_SMT_TRIG_DIS,
    .ucPin = PAD_45,
    .ucFunc = PAD45_FUNC_SEL_SW_DP_CLK,
    .ucPull = PAD_NOPULL,
  },

  { //setup debug input pins - S3_SW_DP_IO
    .ucMode = PAD_MODE_INPUT_EN,
    .ucCtrl = PAD_CTRL_SRC_OTHER,
    .ucDrv = PAD_DRV_STRENGHT_4MA,
    .ucSpeed = PAD_SLEW_RATE_SLOW,
    .ucSmtTrg = PAD_SMT_TRIG_DIS,
    .ucPin = PAD_44,
    .ucFunc = PAD44_FUNC_SEL_SW_DP_IO,
    .ucPull = PAD_NOPULL
  },

  {
    // Note: Pin 9 is a bootstrapping pin, description relates to default config
  ...
  // LEDS
   { // setup blue LED (not present on board, routed to IO header)
    .ucPin = PAD_28,
  ...
  { // setup green LED (not present on board, routed to IO header)
    .ucPin = PAD_21,
  ...
  { // setup red LED
   ...
   { // Pad 33 -- Test clock (not escaped from board, can this be placed on an alternate pin?)
    .ucPin = PAD_33,
